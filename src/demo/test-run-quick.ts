...
export async function _run_test_quick(name: string, options: any, timeout?: number) {
    const fs = await VirtualFileSystem.new([
        { source: `/projects/${name}.zip`, prefix: "C:/Projects" },
    ...
    const miss = findMissingCommandsRecursive(prj.rootClassName, classes);
...
export async function _show_scheme(name: string, className?: string) {
    const fs = await VirtualFileSystem.new([{ source: `/projects/${name}.zip` }, { source: "/data/library.zip" }]);
